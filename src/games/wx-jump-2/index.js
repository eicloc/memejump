...
class JumpGameWorld {
  ...
  initStage() {
    ...
    // 计算相机应该放在哪里
    const cameraInitalPosition = (this.cameraInitalPosition =
    ...
    const lightInitalPosition = (this.lightInitalPosition = {
      x: -300,
      y: 600,
      z: 200,
    });
  ...
  // 创建盒子
  createProp(enterHeight = 100, index = -1) {
    const {
      propCreator,
      propHeight,
    ...
    const prop = new Prop({
      ...
      stage,
      // 头2个盒子用第一个创造器生成
      body: propCreator.createProp(
        index === -1 ? (length < 3 ? 0 : index) : index
      ),
      height: propHeight,
      prev: currentProp,
      enterHeight,
  ...
  computeSafeClearLength() {
    ...
    this.safeClearLength =
      Math.ceil(width / minS) + Math.ceil(height / hypotenuse / 2) + 1;
  }
}
